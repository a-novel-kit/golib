...
| `httpf`       | Both sides of the HTTP wire: the REST boundary logic a handler leans on — mapping domain error sentinels to status codes and writing JSON — and the pooled, traced client a service calls other systems through, with a scripted test server in `httpf/httpftest`. |
...
| `worker`      | Runs the loops that live beside the request path, with the decisions a hand-rolled ticker gets subtly wrong already made: an error keeps the loop alive, a busy tick skips the wait, and loops sharing an interval de-synchronise.                                 |
