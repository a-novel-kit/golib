...
// schemaDropper is the optional capability a postgres.Config offers to remove a schema created
// through DBSchema. The bundled Default implements it; a postgres.Config that does not simply
...
func RunIsolatedTransactionalTest(
	t *testing.T, config postgres.Config, migrations fs.FS, callback TransactionalTestFunc,
) {
	t.Helper()
...
// dbTestOptionsConfig is the capability RunDBTest needs on top of the postgres.Config
...
// database, retrievable with postgres.GetContext.
func RunDBTest(t *testing.T, config postgres.Config, migrations fs.FS, callback TransactionalTestFunc) {
	...
	require.Truef(t, ok,
		"RunDBTest requires a postgres.Config exposing Options() []pgdriver.Option (e.g. postgrespresets.Default)")
